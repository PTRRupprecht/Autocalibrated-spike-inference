...
% x 1. Simulate calcium trace with double exponential template
% x 2. Using gradient descent to optimize the simulation
% x 3. Plot comparison with real calcium trace


%% GD Modeling for a single neuron (evaluate the performance of GD)


% load the data
data = load('CAttached_jGCaMP8s_472181_2_mini.mat');
...
for i = 1:length(CAttached)
    ...
    % compute error (MSE)
    error = mean((measured_trace - gd).^2);

    % plot the optimized simulated trace
    subplot(nRows, nCols, i); 
    plot(time, measured_trace, 'b', time, gd, 'g');
    legend('Measured', 'Simulated');
    xlabel('Time (s)');
    ylabel('ΔF/F');
    title(['Measured vs Simulated Calcium Trace for recording ', num2str(i)]);
...
figure;
plot(time, simulated_trace, 'r', time, gd, 'g');
legend('Raw Modeling', 'Gradient Descent');
xlabel('Time (s)');
ylabel('ΔF/F');
xlim([0,160]);
ylim([-0.5,10]);
...
subplot(2,1,1);
plot(time, measured_trace, 'b', time, simulated_trace, 'r');
legend('Measured', 'Raw Simulation');
xlabel('Time (s)');
ylabel('ΔF/F');
xlim([0, 160]);
ylim([-0.5, 10]);
...
subplot(2,1,2);
plot(time, measured_trace, 'b', time, gd, 'g');
legend('Measured', 'Gradient Descent');
xlabel('Time (s)');
ylabel('ΔF/F');
xlim([0, 160]);
ylim([-0.5, 10]);

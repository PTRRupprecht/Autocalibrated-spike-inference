...
% x 1. Detect events and compute their integrated spike rates
% x 2. Fit spike rates per event with hybrid GMM and detect single spikes
% x 3. Calculate error metrics based on ground truth data
% x 4. Plot statistical analysis
% xx. 'TestPriors' function can evaluate the effect of parameters on the model
...
text(-0.1, 1.05, 'A', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold');
grid on;
...
text(-0.1, 1.05, 'B', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold');
grid on;
...
text(-0.1, 1.05, 'C', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold');
grid on;
...
text(-0.1, 1.05, 'D', 'Units', 'normalized', 'FontSize', 18, 'FontWeight', 'bold');
grid on;
...
figure;
plot(all_spike_rates_GT{6},'r'); 
hold on; 
plot(all_spike_rates{6},'b'); 
plot(all_optimized_rates{6},'g');
xlabel('Event Number');
...
function Visualization(X, model, unitary_amplitude, options)
    ...
    component_colors = cell(length(weights), 1);
    component_idx = 1;
    
    for i = 1:length(weights)
        if weights(i) > options.weight_threshold
            y_i = weights(i) * normpdf(x, means(i), sigmas(i));
            component_colors{i} = cmap(component_idx,:);  % Store color
            plot(x, y_i, 'Color', component_colors{i}, 'LineWidth', 2, ...
    ...
    % get sorted good components and their colors
    good_means = means(good_idx);
    plot_colors = component_colors(good_idx);
    [sorted_means, sort_idx] = sort(good_means);
    sorted_colors = plot_colors(sort_idx);
    
    % bar plot of component means with matched colors
    b = bar(sorted_means);
    for i = 1:length(sorted_means)
        b.FaceColor = 'flat';
        b.CData(i,:) = cell2mat(sorted_colors(i));
    end
    hold on;
    ...
    ylabel('Mean Value');
    legend('Actual Means', 'Expected Integer Multiples', 'Location', 'northwest');
